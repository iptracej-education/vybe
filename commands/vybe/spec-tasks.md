...
- Design document: @.vybe/specs/$ARGUMENTS/design.md
- Spec metadata: @.vybe/specs/$ARGUMENTS/spec.json

**Note**: If this command was called with `-y` flag, both requirements and design are auto-approved (spec.json updated to set requirements.approved=true and design.approved=true). Otherwise, both phases must be approved first via their respective commands followed by `/vybe:spec-tasks $ARGUMENTS -y`.

## Context Analysis

### Complete Spec Context (APPROVED)
- Requirements: @.vybe/specs/$ARGUMENTS/requirements.md
- Design: @.vybe/specs/$ARGUMENTS/design.md
- Current tasks: @.vybe/specs/$ARGUMENTS/tasks.md
- Spec metadata: @.vybe/specs/$ARGUMENTS/spec.json

### Steering Context
- Architecture patterns: @.vybe/steering/structure.md
- Development practices: @.vybe/steering/tech.md
...
Create tasks.md in the language specified in spec.json (check `@.vybe/specs/$ARGUMENTS/spec.json` for "language" field):
...
📄 Generated: .vybe/specs/feature-name/tasks.md
